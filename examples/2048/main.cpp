...
struct Game2048 : Screen
{
	Game2048()
	{
		...
		auto board = std::shared_ptr<ComponentGrid>(
			new ComponentGrid(
				entities,
				Vector2Int(4, 4),
				[this]()
		...
		board->setSizeMode(entities, Component::SizeMode_FixedAspectRatio);
		board->aspectRatio = 1.0f;
		board->setRelativeSize(entities, Vector2(1.0f, 1.0f));
		board->setOffsetSize(entities, Vector2(-100.0f, -100.0f));
